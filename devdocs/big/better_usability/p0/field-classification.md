...
| `DesiredEndpoint.vpn_dns_name` / `protocol` / `port`; `DesiredIPRange.generate_dnsmasq` | none — trivially optional Override fields with no derivation algorithm | Use as given; no computation, no candidate set, no precedence to specify | N/A — absence is simply "not applicable/not set", never an error | `None`/`None`/`None`/`False` respectively | The field itself | source=`override` when set, otherwise absent (no derived value is ever asserted for these) |
