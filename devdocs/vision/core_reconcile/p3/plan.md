...
- [x] The dashboard is regenerated by the routine command itself (drift computation + render
  in one `nctl dashboard` run) and can re-render any saved payload via `--from`; `nctl drift`
  remains side-effect free.
- [x] nintent 0.7.0 is deployed with exactly the roadmap's additions: reconciliation status
...
- [x] Status push degrades to warnings without blocking generation, and the pushed values
  match the live payload after a `nctl dashboard` run.
- [x] `uv run pytest` passes in nctl including the renderer/CLI/push suites; nintent's suite
