...
Two callers. The window agent calls `write_incident` / `list_incidents`
directly, as its two dedicated tools (`agent_runner.window_tools`) — there is
no shell on that door to run a script from. A human runs the CLI shim:
