...
| mTLS conformance | superproject root | `uv run --project cagent pytest -q devtests/test_strategy/test_mtls_conformance.py` | A cagent TLS/ledger trust | local uv (cagent's `cryptography` dev dep); no skips | pytest temp CA/keys/ledger/loopback TLS server; fixture stops exact server | required when cagent's mTLS/ledger/DesiredNode-check boundary changes |
